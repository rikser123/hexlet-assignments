...
public final class App {
    ...
    public static Javalin getApp() {
        ...
        app.post("/articles", ctx -> {
            var currentTitle = ctx.formParam("title");
            var currentContent = ctx.formParam("content");

            try {
                var title = ctx.formParamAsClass("title", String.class)
                        .check(value -> value.length() > 2, "Название не должно быть короче двух символов")
                        .check(value -> {
                            var existedArticle = ArticleRepository.findByTitle(currentTitle);
                ...
                var page = new BuildArticlePage(currentTitle, currentContent, e.getErrors());
                ctx.status(422);
                ctx.render("articles/build.jte", model("page", page));
            }
        });
